...
function App() {
  ...
  const SendAwait = async () => {
    const res = await fetch("https://api.themoviedb.org/4/account/5f198024a6d931003787052f/lists" , {
      headers:header
    })
  ...
  return (
    <div>
      <button onClick = {SendAwait}>Read</button>
      {list.length > 0 && list.map(s => <div key={s.id}>{s.name ?? s.title}</div>)}  
